...
export function plainEncrypt(obj) {
  ...
  keys.forEach(v => obj[`${v}_text`] = (plainList.includes(v) ? plainString(obj[v]) : obj[v]))
  return obj
}
...
export function dateDuration(date) {
  ...
  const unit = { year: '年', month: '月', day: '天', hour: '小时', minute: '分', second: '秒' },
    ...
    second = parseInt(n2str(date % 60), 10),
    minute = parseInt(n2str(date / 60 % 60), 10),
    hour = parseInt(n2str(date / 60 / 60 % 24), 10),
    day = parseInt(n2str(date / 60 / 60 / 24 % 30), 10),
    month = parseInt(n2str(date / 60 / 60 / 24 % 365 / 30), 10),
    year = parseInt(n2str(date / 60 / 60 / 24 / 365), 10),
